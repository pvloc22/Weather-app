...
import '../../../../core/local_storage/search_history_service.dart';
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  final emailRepository = EmailSubscriptionRepository();
  ...
  HomeBloc() : super(HomeInitialState()) {
    ...
    on<CheckSubscriptionStatusEvent>(_checkSubscriptionStatus);
  }
  ...
  void _searchCity(SearchCityEvent event, Emitter<HomeState> emit) async {
    ...
    try {
      ...
      await searchHistoryService.addSearchQuery(query: event.cityName, cityName: standardizedCityName);

      //  Check if there's a cache for this city
      final WeatherResponse? weatherResponseCached = await serviceWeatherPreferences.getCachedWeatherResponse(
        standardizedCityName,
      );
      ...
      if (errorMessage.contains('Network error')) {
        ...
        emit(HomeErrorState(errorMessage: 'City not found. Please try a different location.', errorType: errorType));
      } else if (errorMessage.contains('Server error')) {
        // Server error
        emit(
          HomeErrorState(errorMessage: 'Server error occurred. Please try again later.', errorType: GENERAL_ERROR_TYPE),
        );
        ...
        // Generic error
        emit(
          HomeErrorState(errorMessage: 'Failed to search weather data: $errorMessage', errorType: GENERAL_ERROR_TYPE),
        );
      }
    }
  }
  ...
  void _loadMoreForecast(LoadMoreForecastEvent event, Emitter<HomeState> emit) async {
    ...
    try {
      ...
      final standardizedCityName = event.cityName.replaceAll(RegExp(r'[\[\]\(\)\{\}]'), '');
      ...
      if (errorMessage.contains('Network error')) {
        ...
        errorType = CITY_NOT_FOUND_ERROR_TYPE;
        emit(
          HomeErrorMoreState(errorMessage: 'City not found. Please try a different location.', errorType: errorType),
        );
      } else if (errorMessage.contains('Server error')) {
        // Server error
        emit(
  ...
  void _loadCurrentWeather(RequireCurrentLocationEvent event, Emitter<HomeState> emit) async {
    ...
    try {
      ...
      if (coordinates != null && coordinates.length == 2) {
        ...
        try {
          ...
          emit(
            HomeErrorState(
              errorType: GENERAL_ERROR_TYPE,
              errorMessage: 'Failed to fetch weather for your location: ${e.toString()}',
            ),
          );
        }
      } else {
      ...
      } else if (e.toString().contains('POSITION_UNAVAILABLE') || e.toString().contains('unavailable')) {
  ...
  void _checkSubscriptionStatus(CheckSubscriptionStatusEvent event, Emitter<HomeState> emit) async{
    try {
      final isSubscribed = await emailRepository.isSubscribed();
      emit(HomeStatusSubscriptionState(status: isSubscribed));
    } catch (e) {
      emit(HomeStatusSubscriptionState(status: false));
    }
  }
}
